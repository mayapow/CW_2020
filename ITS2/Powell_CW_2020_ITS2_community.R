...
#These samples were collected from corals in Curaçao
#Across three seasonal timepoints in March 2020, Nov 2020, and Nov 2021
#And were collected between two extreme inland bays and two milder fringing reefs
...
#here() starts at /Users/mayapowell/Documents/Castillo_Lab/CW_2020/CW_2020_Repo

####INITIAL DATA PROCESSING####
# #Commented out because only needed once initially - read in all processed data below
# #Check for low read samples
# counts <- read.csv(here("ITS2",'symportal_profile_counts.csv'),header=TRUE,row.names=1,check.names=FALSE)
# plot(rowSums(counts)) 
...
# 
# #sample dataframe
# samdf<-read.csv(here("ITS2","sample_info_curacao_2020.csv"))
# head(samdf)
...
# #import taxa info
# taxa <- read.csv(here("ITS2","symportal_taxa.csv"),header=TRUE)
...
# 
# counts.all <- read.csv(here("ITS2","symportal_allcounts.csv"),header = T, row.names=1)
...
# 
# taxa.all <- read.csv(here("ITS2","symportal_taxa_all.csv"),header=TRUE)
# rownames(taxa.all) <- as.factor(taxa.all$ITS2_type)
...
# 
# ##save data
# saveRDS(ps, here("ITS2","ps.its2.RDS"))
# saveRDS(ps.nd, here("ITS2","ps.nd.its2.RDS"))
# saveRDS(ps.all,here("ITS2","ps.all.its2.RDS"))
# saveRDS(ps.nd.all, here("ITS2","ps.nd.all.its2.RDS"))


#####READ IN DATA####
ps <- readRDS(here("ITS2","ps.its2.RDS")) #includes ALL DATA
ps.all <- readRDS(here("ITS2","ps.all.its2.RDS")) #includes ALL DATA
ps.nd <- readRDS(here("ITS2","ps.nd.its2.RDS")) #removes duplicate samples
ps.nd.all <- readRDS(here("ITS2","ps.nd.all.its2.RDS")) #removes duplicate samples

#create relative dataframes using the no-duplicate (nd) dataframes
ps.rel <- transform_sample_counts(ps.nd, function(x) x / sum(x))
ps.all.rel <- transform_sample_counts(ps.nd.all, function(x) x / sum(x))
...
ps.ss.all.rel <- subset_samples(ps.all.rel, host_species=="siderea")
ps.sr.all.rel <- subset_samples(ps.all.rel, host_species=="radians")
ps.pp.all.rel <- subset_samples(ps.all.rel, host_species=="porites")
