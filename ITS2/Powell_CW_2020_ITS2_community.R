...
setwd('~/Documents/Castillo Lab/CW_2020/CW_2020_ITS2')
#load libraries
library(phyloseq)
library('ggplot2')
library('Rmisc')
...
ps.nd <- readRDS("ps.nd.all.its2.RDS")
...
ps.rel <- transform_sample_counts(ps.nd, function(x) x / sum(x))
gg.bar <- plot_bar(ps.rel,"sample_full",fill="ITS2_type")+
...
color_obj_prep_test <- create_color_dfs(mdf_prep, selected_groups = c("A","B","C","D"), group_level = "Clade", subgroup_level = "Majority_ITS2_sequence", cvd = TRUE)
...
# Create a color object for the specified data
color_obj_prep_test <- create_color_dfs(mdf_prep, selected_groups = c("A","B","C","D"), group_level = "Clade", subgroup_level = "ITS2_type", cvd = TRUE)
